...
def oneTimeInit():
	...
	ratingEdgeList = []
	tempEdgeList = []
	for line in open('eval_ntwk_food_friends/eval_ntwk_edge_list_phoenix2.txt'):
		tempEdgeList.append(tuple(line.strip().split(' ')))

	for t in tempEdgeList:
		edge = int(t[0]), int(t[1])
		ratingEdgeList.append(edge)
		# attrAverages[edge] = 0.0

	# print ratingEdgeList
	# return ratingEdgeList, attrAverages
...
def runTrial(ratingEdgeList, userMap):	
	...
	for thresholdVal in thresholdVals: # for each potential threshold
		...
		for edge in randomEdges:
			...
			attrVals.append([edge[0], edge[1], attrCompScore])
